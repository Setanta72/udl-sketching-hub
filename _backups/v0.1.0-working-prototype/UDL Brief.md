...
- **Challenge:** This audience is not uniform. The site's architecture **must** be designed to explicitly support two primary learner paths:
    
    1. **The "Guided Path":** For learners who aim to achieve **Proficiency**. They are comfortable using aids (rulers, templates, guides) to produce accurate, high-fidelity work and fulfill course requirements.
        
    2. **The "Mastery Path":** For learners who aim for **Freehand Mastery**. They want to use minimal aids and develop the high-level, gestural skill required for rapid ideation.
...
## 4. Key Architectural Feature: "Learning Mode" Toggle

To address the Core Challenge, the site must feature a site-wide "Learning Mode" toggle.

- **UI Element:** A simple toggle/switch in the site header, e.g., `[ Guided Path ] <-> [ Mastery Path ]`
    
- **Default State:** "Guided Path"
    
- **Function:** This choice must be saved in the browser's `localStorage`. This toggle **must dynamically filter or re-prioritize the content** presented on the Module Pages.
    
- **UDL Rationale:** This directly supports _Action & Expression_ (letting the user choose their method) and _Engagement_ (providing an appropriate, custom-tailored level of challenge).
...
1. **Homepage (`/`)**
    ...
    - **Purpose:** Introduce the UDL philosophy and the "Learning Mode" toggle.
        
    - **Content:**
        
        - Brief intro to the UDL approach.
            
        - A call-to-action to select a "Learning Mode."
...
3. **About Page (`/about`)**
    ...
    - **Content:** "What is UDL?", "How to use the Mastery Rubrics", "How to use the Learning Modes", "How to use Physical vs. Digital modes."
        

## 6. Module Page Template: Detailed Specification

This is the most critical component. The template _must_ be populated from external Markdown files and _must_ react to the "Learning Mode" toggle.
...
### 6.3. Section 3: Skills & Drills

- **Content:** A checklist of drills.
    
- **DYNAMIC BEHAVIOR (CRITICAL):**
    
    - **"Guided Path" Mode:** This view prioritizes aided drills. E.g., "Drill: Explicit Cube Studies (Aided)" is shown as the primary drill. "Drill: Gestural Cubes (Freehand)" is shown as an optional "challenge."
        
    - **"Mastery Path" Mode:** This view prioritizes freehand drills. E.g., "Drill: Gestural Cubes (Freehand)" is the primary drill. "Drill: Explicit Cube Studies (Aided)" is listed as a "verification" exercise.
        
    - _Developer Note:_ This can be achieved by adding a data attribute to the list items in the source file (e.g., `data-path="guided"` or `data-path="mastery"`) and using CSS/JS to filter them.
        

### 6.4. Section 4: Modes of Expression (Physical vs. Digital)

- **Content:** A prominent tabbed interface: **"Physical (Pen & Paper)"** vs. **"Digital (Tablet & Software)"**.
    
- **Feature:** This is a simple content toggle, separate from the "Learning Mode" toggle. It provides tips relevant to the learner's chosen toolset.
...
### 6.7. Section 7: Mastery Rubric

- **Content:** An interactive self-assessment checklist.
    
- **Feature:** The learner can click "Novice", "Proficient", or "Mastery" to self-identify. This choice **must be saved to `localStorage`**.
    
- **DYNAMIC BEHAVIOR (Subtle but important):** The _language_ of the rubric should adapt to the "Learning Mode".
    
    - **"Guided Path":** "Mastery" = "Can use aids to produce _high-fidelity, accurate drawings_."
        
    - **"Mastery Path":** "Mastery" = "Can use _minimal aids_ to produce _fast, gesturally-accurate drawings_."
